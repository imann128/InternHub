...
- **Postgres Row-Level Security** on all 9 tenant-owned tables, as a database-enforced backstop behind the existing application-layer tenant filtering: a policy on each table requires `organization_id` to match a per-connection `app.current_org_id` session setting. A request's connection is pinned to its organization via `AsyncLocalStorage` (`backend/src/config/db.js`), checked out per-request by `authMiddleware` and per-organization-iteration by the cron jobs and retention cleanup. Two `SECURITY DEFINER` functions (`intern_find_by_email`, `intern_email_exists`) provide a narrow, explicit bypass for the two lookups that must legitimately search across all organizations (login, email-uniqueness). Only provides real protection once DB role separation above is adopted, since RLS never applies to a table's owner.
